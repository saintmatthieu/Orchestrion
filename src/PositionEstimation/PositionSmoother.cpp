...
PositionSmoother::PositionSmoother(double memory, std::size_t maxKnots,
                                   double maxWindowMs)
    : _memory(memory), _maxKnots(maxKnots), _maxWindowMs(maxWindowMs)
{
}

void PositionSmoother::addObservation(double realTime, double musicalPos)
{
...
void PositionSmoother::evalAt(double realTime, double &position,
...
double PositionSmoother::positionAt(double realTime) const
{
...
double PositionSmoother::velocityAt(double realTime) const
{
  double position = 0.0, velocity = 0.0;
  evalAt(realTime, position, velocity);
  return velocity;
...
std::vector<PositionSmoother::Residual> PositionSmoother::residuals() const
{
  ...
  for (std::size_t i = 0; i < _knots.size(); ++i)
    result.push_back(
        {_knots[i].time, _obs[i].pos - _knots[i].position, _knots[i].velocity});
  return result;
}
