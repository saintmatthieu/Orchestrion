...
namespace dgk
{
//! Companion to PositionTracker: fed the same sparse onsets, it maintains the
...
//!
//! Where PositionTracker answers "where is the performer now?" causally (its
//! tempo trace is a staircase: constant between onsets, nudged at each one),
//! this class re-fits the whole recent past on every onset: a Kalman forward
//! pass over the window followed by a Rauch–Tung–Striebel backward pass yields
//! the posterior state (position, tempo) at every onset time, and between
//! onsets the posterior mean is the cubic Hermite bridge between adjacent knot
//! states — piecewise cubic in position, C¹, i.e. the (windowed) cubic
//! smoothing spline. Each new onset bends the recent knots a little; the
//! influence dies off like memoryⁿ with the number of onsets in between, so
//! values a few onsets back are essentially final ("fixed-lag" smoothing).
//! Uses: a spline-true tempo visualization, a delayed-but-smooth scroll anchor,
//! and later rhythm grading against the settled curve.
//!
//! Same conventions as PositionTracker: units are caller-defined, one tuning
//! knob \p memory (γ), no dependencies. Cost is O(window) 2×2 arithmetic per
//! onset.
class PositionSmoother
{
public:
  //! \p memory is γ ∈ (0,1) as in PositionTracker: higher = smoother. It maps
  //! to the process/measurement-noise ratio of the underlying state-space
  //! model. \p maxKnots / \p maxWindowMs bound the sliding window (the defaults
  //! suit live following); pass large values for a whole-take offline re-fit.
  explicit PositionSmoother(double memory = 0.6, std::size_t maxKnots = 64,
