...
PositionTracker::PositionTracker(double memory)
...
void PositionTracker::advanceTo(double realTime)
{
...
void PositionTracker::addObservation(double realTime, double musicalPos)
{
  if (_nObs == 0)
  {
...
void PositionTracker::heartbeat(double realTime)
{
  if (_nObs < 2)
    return;
  const double dt = realTime - _lastTime;
...
double PositionTracker::baseAt(double realTime) const
{
  return _p + _v * (realTime - _lastTime);
}

double PositionTracker::offsetAt(double realTime) const
{
...
double PositionTracker::positionAt(double realTime) const
{
  if (_nObs < 2)
