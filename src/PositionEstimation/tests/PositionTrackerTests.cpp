...
// Drive the tracker as its consumers do: onsets are fed at
// their (caller-supplied) times while heartbeat() is sampled at ~60 fps, and
// the per-frame tempo readout (speed()*bpmFactor) is collected once the tracker
// is ready.
std::vector<double> simulateBpm(const std::vector<double> &onsetTimesMs,
                                double tickStep)
{
  PositionTracker tracker; // default memory, as the follower uses
...
TEST(PositionTrackerTests, ConstantTimingYieldsConstantTempo)
{
...
TEST(PositionTrackerTests, TimingJitterShowsInTempo)
{
