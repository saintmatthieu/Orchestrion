...
#include "PositionEstimation/PositionSmoother.h"
...
TEST(PositionSmootherTests, ConstantTimingYieldsConstantSmoothedTempo)
{
...
TEST(PositionSmootherTests, SmoothsTimingJitterBetterThanTheFilter)
{
...
TEST(PositionSmootherTests, LinearTempoRampLeavesTinyResiduals)
{
  ...
  for (int k = 0; k < count; ++k)
  {
    ...
    if (k > count / 2 && tracker.ready())
    {
      ...
      causalBiasMs += std::abs(tracker.predictionError(t, z)) / tracker.speed();
      ++causalCount;
    }
...
TEST(PositionSmootherTests, SmoothedCurveIsContinuousAcrossKnots)
{
  PositionSmoother smoother;
