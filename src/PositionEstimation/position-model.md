# Position estimation — the maths

This document records the mathematics behind `PositionTracker` and why the design
...
implementation in `PositionTracker.{h,cpp}` and `PositionSmoother.{h,cpp}`.
`PositionEstimator` is the layer above: one of each per hand, turning their
output into the two answers consumers ask for — where a hand is now, and how
far each of its onsets fell from the performer's own smooth curve.

The quantity estimated is the performer's **position** in the score; tempo is
simply its derivative, which is why one filter answers both questions.

Notation: real (wall-clock) time `t` in ms; musical position `s` in some linear
musical coordinate (page-logical x for on-score positions, or score ticks for
tempo); tempo / velocity `v = ds/dt`. Onsets (played notes) arrive at times `t_k` with
...
- it is **model-agnostic** and reusable (rhythm grading — the per-onset
  residual is the timing error — and tempo-following auto-play).
...
(`PositionTrackerTests.ConstantTimingYieldsConstantTempo`: spread `< 10⁻⁶`).
...
## 7. Two coordinates: position (x) vs tempo readout (ticks)

The model is coordinate-agnostic; we run it in **two** coordinates per hand
(staff), because they answer different questions.

- **Position** is tracked in **page-logical x** (`onsetX`): `positionAt → x`,
  which is where a played note *is* on the page — what the on-score overlays
  need. Note that layout x is _not_ proportional to musical time (MuseScore's
  note spacing is sub-linear in duration), so x-speed varies even at a
...
Repeats are handled by feeding each tracker a monotonic coordinate: the
position tracker folds each backward jump into an accumulating x-offset (kept monotonic,
...
## 7b. Getting the actual spline: fixed-lag smoothing (`PositionSmoother`)
...
`PositionSmoother` implements this over a sliding window of recent onsets. Per
...
- **Scroll**: not a consumer of this model. The page turn follows *events* —
  it puts the next note to play back on the anchor once it has drifted far
  enough right — which needs score geometry and time, no tempo estimate. It
  lives in `OrchestrionNotation/view/ScoreFollower`, and the reasoning for
  turning pages instead of following a position continuously is documented
  there. An earlier design did pin the viewport to the smoothed position
  `smoothDelayIntervals` onsets back; the constants that served it
  (`anchorFrac`, the anchor's continuity offset) went with it.
...
govern the idle and transient policies.
