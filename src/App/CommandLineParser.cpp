...
namespace
{
QStringList prepareArguments(int argc, char **argv)
{
  QStringList args;
...
void CommandLineParser::init()
{
  ...
  m_parser.addPositionalArgument("scorefiles", "The files to open",
                                 "[scorefile...]");

  m_parser.addOption(
      QCommandLineOption("long-version", "Print detailed version information"));
  ...
  m_parser.addOption(
      QCommandLineOption({"T", "trim-image"},
                         "Use with '-o <file>.png' and '-o <file.svg>'. Trim "
                         "exported image with specified margin (in pixels)",
                         "margin"));

  m_parser.addOption(QCommandLineOption(
      {"b", "bitrate"}, "Use with '-o <file>.mp3', sets bitrate, in kbps",
      "bitrate"));

  m_parser.addOption(QCommandLineOption(
      "template-mode",
      "Save template mode, no page size")); // and no platform and creationDate
                                            // tags
  m_parser.addOption(QCommandLineOption(
      {"t", "test-mode"},
  ...
  m_parser.addOption(
      QCommandLineOption("session-type", "Startup with given session type",
                         "type")); // see StartupScenario::sessionTypeTromString

  // Converter mode
  m_parser.addOption(
      QCommandLineOption({"r", "image-resolution"},
                         "Set output resolution for image export", "DPI"));
  m_parser.addOption(
      QCommandLineOption({"j", "job"}, "Process a conversion job", "file"));
  m_parser.addOption(QCommandLineOption(
      {"o", "export-to"},
      "Export to 'file'. Format depends on file's extension", "file"));
  m_parser.addOption(
      QCommandLineOption({"F", "factory-settings"}, "Use factory settings"));
  m_parser.addOption(QCommandLineOption(
      {"R", "revert-settings"},
  ...
  m_parser.addOption(
      QCommandLineOption({"P", "export-score-parts"},
                         "Use with '-o <file>.pdf', export score and parts"));
  m_parser.addOption(QCommandLineOption(
      {"f", "force"}, "Use with '-o <file>', ignore warnings reg. score being "
                      "corrupted or from wrong version"));

  m_parser.addOption(QCommandLineOption(
      "score-media", "Export all media (excepting mp3) for a given score in a "
                     "single JSON file and print it to stdout"));
  m_parser.addOption(QCommandLineOption(
      "highlight-config", "Set highlight to svg, generated from a given score",
      "highlight-config"));
  m_parser.addOption(QCommandLineOption(
      "score-meta",
      "Export score metadata to JSON document and print it to stdout"));
  m_parser.addOption(QCommandLineOption(
      "score-parts", "Generate parts data for the given score and save them to "
                     "separate mscz files"));
  m_parser.addOption(QCommandLineOption(
      "score-parts-pdf", "Generate parts data for the given score and export "
                         "the data to a single JSON file, print it to stdout"));
  m_parser.addOption(
      QCommandLineOption("score-transpose",
                         "Transpose the given score and export the data to a "
                         "single JSON file, print it to stdout",
                         "options"));
  m_parser.addOption(QCommandLineOption(
      "source-update", "Update the source in the given score"));

  m_parser.addOption(
      QCommandLineOption({"S", "style"}, "Load style file", "style"));

  m_parser.addOption(QCommandLineOption(
      "sound-profile",
      "Use with '-o <file>.mp3' or with '-j <file>', override the sound "
      "profile in the given score(s). "
      "Possible values: \"MuseScore Basic\", \"Muse Sounds\"",
      "sound-profile"));

  // MusicXML
  m_parser.addOption(
      QCommandLineOption("musicxml-use-default-font",
                         "Apply default typeface (Edwin) to imported scores"));
  m_parser.addOption(
      QCommandLineOption("musicxml-infer-text-type",
  ...
  //                                          "View mode [paged-float,
  //                                          paged-original,
  //                                          paged-float-height, pano, auto].
  //                                          Auto (default) will choose the
  //                                          best mode according to number of
  //                                          instruments etc... Will show piano
  //                                          for piano score only", "auto"));
  // not implemented
  //    m_parser.addOption(QCommandLineOption("piano", "Show Piano, works only
  //    if one part and not auto or float modes"));
  //    m_parser.addOption(QCommandLineOption("piano-position", "Show Piano top
  //    or bottom. Default bottom", "bottom"));
  m_parser.addOption(QCommandLineOption(
      "resolution", "Resolution [2160p, 1440p, 1080p, 720p, 480p, 360p]",
      "1080p"));
  m_parser.addOption(
      QCommandLineOption("fps", "Frame per second [60, 30, 24]", "24"));
  m_parser.addOption(QCommandLineOption(
      "ls", "Pause before playback in seconds (3.0)", "3.0"));
  m_parser.addOption(QCommandLineOption(
      "ts", "Pause before end of video in seconds (3.0)", "3.0"));
#endif

  m_parser.addOption(QCommandLineOption(
      "gp-linked", "create tabulature linked staves for guitar pro"));
  m_parser.addOption(QCommandLineOption(
  ...
  // Diagnostic
  m_parser.addOption(
      QCommandLineOption("diagnostic-output", "Diagnostic output", "output"));
  m_parser.addOption(QCommandLineOption(
      "diagnostic-gen-drawdata", "Generate engraving draw data", "scores-dir"));
  m_parser.addOption(QCommandLineOption("diagnostic-com-drawdata",
                                        "Compare engraving draw data"));
  m_parser.addOption(QCommandLineOption("diagnostic-drawdata-to-png",
                                        "Convert draw data to png", "file"));
  m_parser.addOption(QCommandLineOption("diagnostic-drawdiff-to-png",
                                        "Convert draw diff to png"));

  // Autobot
  m_parser.addOption(QCommandLineOption(
      "test-case", "Run test case by name or file", "nameOrFile"));
  m_parser.addOption(QCommandLineOption("test-case-context",
                                        "Set test case context by name or file",
                                        "nameOrFile"));
  m_parser.addOption(QCommandLineOption(
      "test-case-context-value", "Set test case context value", "value"));
  m_parser.addOption(
      QCommandLineOption("test-case-func", "Call test case function", "name"));
  m_parser.addOption(QCommandLineOption(
      "test-case-func-args", "Call test case function args", "args"));

  // Audio plugins
  m_parser.addOption(
      QCommandLineOption("register-audio-plugin",
                         "Check an audio plugin for compatibility with the "
                         "application and register it",
                         "path"));
  m_parser.addOption(QCommandLineOption(
      "register-audio-plugin-out",
      "Write the audio plugin registration result to this file", "path"));

  // Internal
  m_parser.addOption(
      internalCommandLineOption("score-display-name-override",
                                "Display name to be shown in splash screen for "
                                "the score that is being opened",
                                "name"));
}
...
void CommandLineParser::parse(int argc, char **argv)
{
  ...
  { return muse::io::Dir::fromNativeSeparators(path).toQString(); };
  ...
  if (m_parser.isSet("M"))
  {
    m_options->importMidi.operationsFile =
        fromUserInputPath(m_parser.value("M"));
  }
  ...
  if (m_parser.isSet("b"))
  {
    ...
    {
      LOGE() << "Option: -b not recognized bitrate value: "
             << m_parser.value("b");
    }
  }
  ...
  {
    m_options->runMode = muse::IApplication::RunMode::AudioPluginRegistration;
    m_options->audioPluginRegistration.pluginPath =
        fromUserInputPath(m_parser.value("register-audio-plugin"));
    m_options->audioPluginRegistration.outputFile =
        fromUserInputPath(m_parser.value("register-audio-plugin-out"));
  }
  ...
  if (m_parser.isSet("o"))
  {
    m_options->runMode = muse::IApplication::RunMode::ConsoleApp;
    ...
    else
    {
      if (scorefiles.size() > 1)
      {
        LOGW() << "Option: -o multiple input files specified; processing only "
                  "the first one";
      }
      m_options->converterTask.inputFile = scorefiles[0];
      m_options->converterTask.outputFile =
          fromUserInputPath(m_parser.value("o"));
    }
  }
  ...
  if (m_parser.isSet("score-media"))
  {
    ...
    {
      m_options->converterTask
          .params[CommandOptions::ParamKey::HighlightConfigPath] =
  ...
  if (m_parser.isSet("score-transpose"))
  {
    ...
    m_options->converterTask.inputFile = scorefiles[0];
    m_options->converterTask
        .params[CommandOptions::ParamKey::ScoreTransposeOptions] =
  ...
  if (m_parser.isSet("source-update"))
  {
    ...
    if (args2.size() >= 2)
    {
      m_options->converterTask.params[CommandOptions::ParamKey::ScoreSource] =
          args2[1];
    }
    else
    {
  ...
  if (m_parser.isSet("score-video"))
  {
    ...
    {
      m_options->exportVideo.resolution =
          m_parser.value("resolution").toStdString();
    }
  ...
  if (m_parser.isSet("f"))
  {
    m_options->converterTask.params[CommandOptions::ParamKey::ForceMode] = true;
  }

  if (m_parser.isSet("S"))
  {
    m_options->converterTask.params[CommandOptions::ParamKey::StylePath] =
        fromUserInputPath(m_parser.value("S"));
  }

  if (m_parser.isSet("sound-profile"))
  {
    m_options->converterTask.params[CommandOptions::ParamKey::SoundProfile] =
  ...
  if (m_options->runMode == muse::IApplication::RunMode::ConsoleApp)
  {
  ...
  {
    m_options->runMode = muse::IApplication::RunMode::ConsoleApp;
    m_options->autobot.testCaseNameOrFile =
        fromUserInputPath(m_parser.value("test-case"));
  }

  if (m_parser.isSet("test-case-context"))
  {
    m_options->autobot.testCaseContextNameOrFile =
        fromUserInputPath(m_parser.value("test-case-context"));
  }

  if (m_parser.isSet("test-case-context-value"))
  {
    m_options->autobot.testCaseContextValue =
        m_parser.value("test-case-context-value");
  }
  ...
  // Startup
  if (m_options->runMode == muse::IApplication::RunMode::GuiApp)
  {
    if (!scorefiles.isEmpty())
    {
      m_options->startup.scoreUrl = QUrl::fromUserInput(
          scorefiles[0], QDir::currentPath(), QUrl::AssumeLocalFile);
    }

    if (m_parser.isSet("score-display-name-override"))
    {
      m_options->startup.scoreDisplayNameOverride =
          m_parser.value("score-display-name-override");
    }
  }
}
...
std::shared_ptr<CommandOptions> CommandLineParser::options() const
{
  return m_options;
}

CommandOptions::ConverterTask CommandLineParser::converterTask() const
{
  return m_options->converterTask;
}

CommandOptions::Diagnostic CommandLineParser::diagnostic() const
{
  return m_options->diagnostic;
}

CommandOptions::Autobot CommandLineParser::autobot() const
{
  return m_options->autobot;
}

CommandOptions::AudioPluginRegistration
CommandLineParser::audioPluginRegistration() const
{
...
{
  if (muse::BaseApplication::appUnstable())
  {
    printf("Orchestrion\nUnstable Prerelease for Version %s; "
           "Build %s\n",
           muse::BaseApplication::appVersion().toStdString().c_str(),
           muse::BaseApplication::appRevision().toStdString().c_str());
  }
  else
  {
    printf("Orchestrion; Version %s; Build %s\n",
           muse::BaseApplication::appVersion().toStdString().c_str(),
           muse::BaseApplication::appRevision().toStdString().c_str());
  }
}
