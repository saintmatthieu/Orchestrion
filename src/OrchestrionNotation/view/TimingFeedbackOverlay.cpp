...
double
unitedItemsCenterX(const std::vector<mu::engraving::EngravingItem *> &items,
                   double fallback)
{
  if (items.empty())
    return fallback;
...
// The rolling stats' horizon: samples older than this drop out (the take's
// own, never-pruned samples are what the score is computed from).
    ...
    : _requestRepaint(std::move(requestRepaint))
{
  ...
  _hoverTimer.callOnTimeout(
      [this]
      {
        if (!_hoveredGauge || _clock.elapsed() - _hoverStartMs > shadowGlideMs)
          _hoverTimer.stop();
        _requestRepaint();
      });
}
...
TimingFeedbackOverlay::gaugeTipAt(const QPointF &logicalPos) const
{
  ...
  const double dx =
      ((1.0 - _warpProgress) * gauge.warpTicks + gauge.errorTicks) * pxPerTick;
  ...
  return GaugeTip{
      info,
      QPointF(left ? bounds.left() - margin : bounds.right() + margin,
              bounds.center().y()),
      left};
}

void TimingFeedbackOverlay::upsertSamples(
    int staff, const std::vector<PositionEstimator::Judgment> &window,
    SampleMap &samples, TakeSampleMap &takeSamples)
{
  const qint64 now = _clock.elapsed();
  auto &staffSamples = samples[staff];
  auto &staffTakeSamples = takeSamples[staff];
  for (const PositionEstimator::Judgment &judgment : window)
  {
    ...
    int staff, const std::vector<PositionEstimator::Judgment> &window)
{
  ...
  for (Gauge &gauge : _gauges)
  {
    ...
    const auto it = std::find_if(window.begin(), window.end(),
                                 [&gauge](const PositionEstimator::Judgment &j)
    ...
    for (RibbonPoint &point : ribbonIt->second)
    {
      const auto it =
          std::find_if(window.begin(), window.end(),
                       [&point](const PositionEstimator::Judgment &j)
...
std::vector<TimingFeedbackOverlay::ScoreMetric>
TimingFeedbackOverlay::takeScoreMetrics() const
{
  const auto sampleCount = [](const TakeSampleMap &samples)
  {
    std::size_t count = 0;
    for (const auto &[staff, staffSamples] : samples)
      count += staffSamples.size();
    return count;
  };
  const auto metric = [](const QString &label, double quantile, double ref,
                         const QString &detail)
  {
    return ScoreMetric{
        label, static_cast<int>(std::lround(scoreOf(quantile, ref))), detail};
  };

  std::vector<ScoreMetric> metrics;
  if (const auto quantile = takeQuantile(_takeSamples))
    metrics.push_back(metric(QStringLiteral("Tempo smoothness"), *quantile,
                             scoreRefMs,
                             QString::fromUtf8("%1 ms typical error · %2 notes")
                                 .arg(qRound(*quantile))
                                 .arg(sampleCount(_takeSamples))));
  if (const auto quantile = takeSyncAbsErrorQuantile())
    metrics.push_back(metric(QStringLiteral("Hand synchronization"), *quantile,
                             scoreRefMs,
                             QString::fromUtf8("%1 ms typical gap · %2 samples")
                                 .arg(qRound(*quantile))
                                 .arg(_takeSyncErrors.size())));
  if (const auto quantile = takeQuantile(_takeDynamicsSamples))
    metrics.push_back(metric(
        QStringLiteral("Dynamics smoothness"), *quantile, dynamicsScoreRef,
        QString::fromUtf8("%1 % typical deviation · %2 notes")
            .arg(qRound(*quantile * 100))
            .arg(sampleCount(_takeDynamicsSamples))));
  return metrics;
}
...
QString TimingFeedbackOverlay::ribbonInfoAt(const QPointF &logicalPos) const
{
  ...
  for (const auto &[staff, allPoints] : _ribbon)
  {
    ...
    for (std::size_t i = 1; i < points.size(); ++i)
    {
      ...
      QString info =
          QString::fromUtf8("%1 bpm").arg(static_cast<int>(std::lround(bpm)));
      const int deviation = static_cast<int>(std::lround(std::abs(warpMs)));
      if (deviation > 0)
                                        ...
                                        const QRectF &viewport) const
{
  ...
  for (const auto &[staff, points] : _ribbon)
  {
    ...
    for (const RibbonPoint &point : points)
    {
      ...
      minX = std::min(minX, x);
      maxX = std::max(maxX, x);
      strokes.back().push_back(
          {x, baseY + point.warpMs * pxPerMs,
           baseY + (point.warpMs + point.errorMs) * pxPerMs, point.errorMs});
    }
    if (minX > maxX)
