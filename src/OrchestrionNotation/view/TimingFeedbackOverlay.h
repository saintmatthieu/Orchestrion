...
//! and re-evaluated as later onsets refine it (see PositionEstimator::Judgment)
...
public:
  ...
  //! identity (the estimator's clock, as carried by its judgments), by which
  ...
  void updateJudgments(int staff,
                       const std::vector<PositionEstimator::Judgment> &window);

  //! One hand-asynchrony sample (see PositionEstimator::Feedback::handSync):
  //! fed into the sync component of the score. Never arrives for single-staff
