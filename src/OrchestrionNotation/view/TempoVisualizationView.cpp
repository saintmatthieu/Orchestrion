...
void TempoVisualizationView::setModel(TempoVizModel *model)
{
  ...
  _model = model;
  if (_model)
    connect(_model, &TempoVizModel::changed, this, [this] { update(); });
  emit modelChanged();
  update();
}
...
void TempoVisualizationView::paint(QPainter *painter)
{
  ...
  if (_model)
  {
    ...
    for (const auto &[staff, pts] : _model->smoothed())
      for (const auto &p : pts)
        tempoMax = std::max(tempoMax, p.bpm);
  }
  ...
  for (const double frac : {0.0, 0.5, 1.0})
  {
    ...
    painter->setPen(QPen(QColor(255, 255, 255, frac == 0.0 ? 90 : 40), 1.0));
  ...
  for (const auto &[staff, pts] : _model->smoothed())
  {
    ...
    for (std::size_t i = 1; i < pts.size(); ++i)
    {
      ...
      painter->drawLine(QPointF(xOf(pts[i - 1].tMs), yOf(pts[i - 1].bpm)),
                        QPointF(xOf(pts[i].tMs), yOf(pts[i].bpm)));
    }
  }
